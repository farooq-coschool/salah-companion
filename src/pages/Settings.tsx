...
import { useSettingsStore, RECITERS, type ReciterId } from '../store/useSettingsStore';
import { Settings as SettingsIcon, MapPin, Globe, BookOpen, Volume2, Navigation, Loader2, ShieldCheck, Palette, Bell, BellOff, Mic2 } from 'lucide-react';
    ...
    >
        ...
        <div className="space-y-8">
          ...
          <div>
            <label className="flex items-center gap-2 text-sm font-bold text-stone-700 mb-2">
              <Mic2 className="w-4 h-4" /> Reciter
            </label>
            <select
              value={reciter}
            ...
            >
              {(Object.keys(RECITERS) as ReciterId[]).map((id) => (
                <option key={id} value={id}>{RECITERS[id]}</option>
              ))}
            </select>
            <p className="mt-3 text-sm text-stone-500 leading-relaxed">
              Applies to ayah-by-ayah recitation and the Adhan sound on Home.
            </p>
          </div>

          <div>
            <label className="block text-sm font-bold text-stone-700 mb-3">Arabic Text Size</label>
            <div className="flex gap-3">
              {(['sm', 'md', 'lg', 'xl'] as const).map((size) => (
                <button
                  key={size}
                  onClick={() => setArabicFontSize(size)}
                  className={`flex-1 py-3 rounded-xl font-bold uppercase text-xs tracking-wide transition-colors ${
                    arabicFontSize === size ? 'bg-[#0B3D2E] text-gold-200' : 'bg-stone-100 text-stone-500 hover:bg-stone-200'
                  }`}
                >
                  {size}
                </button>
      ...
      >
        <div className="flex items-center gap-3 mb-8">
          ...
          <h2 className="text-2xl font-bold text-stone-800 tracking-tight">Appearance</h2>
        </div>
        <label className="block text-sm font-bold text-stone-700 mb-3">Theme</label>
        <div className="flex gap-3">
          {(['light', 'dark', 'system'] as const).map((t) => (
            <button
              key={t}
              onClick={() => setTheme(t)}
              className={`flex-1 py-3 rounded-xl font-bold capitalize transition-colors ${
                theme === t ? 'bg-[#0B3D2E] text-gold-200' : 'bg-stone-100 text-stone-500 hover:bg-stone-200'
              }`}
            >
              {t}
            </button>
      ...
      >
        <div className="flex items-center gap-3 mb-8">
          ...
          <h2 className="text-2xl font-bold text-stone-800 tracking-tight">Prayer Notifications</h2>
        </div>
        <div className="flex items-center justify-between gap-4">
          <p className="text-sm text-stone-500 leading-relaxed max-w-sm">
            Get a browser notification at each prayer time. Only works while this app is open in a tab
            &mdash; there's no server behind it to deliver notifications when the app is closed.
          </p>
          <button
            onClick={handleToggleNotifications}
            className={`shrink-0 px-5 py-3 rounded-xl font-bold transition-colors ${
              notificationsEnabled ? 'bg-emerald-600 text-white' : 'bg-stone-100 text-stone-600 hover:bg-stone-200'
            }`}
          >
